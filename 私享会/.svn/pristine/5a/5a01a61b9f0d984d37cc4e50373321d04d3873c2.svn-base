...
#define kMainScreenHeight self.frame.size.height

@interface DayTopicADView()
{
    //显示图片的滚动视图
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _pageControll = [[UIPageControl alloc] initWithFrame:CGRectMake(0, kMainScreenHeight-40, kMainScreenWidth, 40)];
        [_pageControll addTarget:self action:@selector(pageControllClicked:) forControlEvents:UIControlEventValueChanged];
        _pageControll.currentPage = 0;
        _pageControll.pageIndicatorTintColor = [UIColor yellowColor];
...
- (void)getImgArr:(NSArray *)arr
{
    ...
    _scrollView.contentSize = CGSizeMake(kMainScreenWidth*adNumber, kMainScreenHeight);
    _pageControll.frame = CGRectMake(0, kMainScreenHeight-40, kMainScreenWidth, 40);
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(tapAD:)]) {
        int indedx = _scrollView.contentOffset.x/kMainScreenWidth - 1;
        if (indedx < 0 && haveImg) {
            indedx = 0;
        }
        [_delegate tapAD:indedx];
    }
}
...
- (UIImageView *)createImgViewWithIndex:(NSInteger)index
{
    ...
    if (imageArr.count <= index) {
        imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0 + kMainScreenWidth * index, 0, kMainScreenWidth, kMainScreenHeight)];
        imageView.userInteractionEnabled = YES;
        [_scrollView addSubview:imageView];
        [imageArr addObject:imageView];
    } else {
...
- (void)setImage:(UIImageView *)imgV withUrlString:(UIImage *)string
{
